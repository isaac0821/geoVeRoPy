...
def rndPts(N: int, distr = 'UniformSquareXY', seed = None, **kwargs) -> list:
    ...
    **kwargs: optional
    ...
    if (distr == 'UniformSquareXY'):
        ...
        for n in range(N):
            nodePts.append(_rndPtUniformSquareXY(xRange, yRange))
        ...
        for n in range(N):
            nodePts.append(_rndPtUniformCubeXYZ(xRange, yRange, zRange))
    ...
    # Uniformly sample from a polygon/a list of polygons on the Euclidean space
        ...
        if ('polyXY' in kwargs):
            ...
            for n in range(N):
                nodePts.append(_rndPtUniformPolyXYs(kwargs['polyXY']))
    ...
    # Uniformly sample from the Euclidean space avoiding polygons
        ...
        if ('polyXY' in kwargs):
            for n in range(N):
                nodePts.append(_rndPtUniformAvoidPolyXY(kwargs['polyXY'], xRange, yRange))
        elif ('polyXYs' in kwargs):
            for n in range(N):
                nodePts.append(_rndPtUniformAvoidPolyXYs(kwargs['polyXYs'], xRange, yRange))
    ...
    # Uniformly sample from a circle on the Euclidean space
        ...
        for n in range(N):
            nodePts.append(_rndPtUniformCircleXY(radius, centerXY))
    ...
    # Uniformly sample from the roads/streets within a polygon/a list of polygons from given road networks
        ...
        if ('polyLatLon' in kwargs):
            ...
            nodePts = _rndPtRoadNetworkPolyLatLons(
                N if N != None else len(nodeIDs),
                kwargs['roads'], 
                kwargs['polyLatLons'],
                kwargs['roadClass'] if 'roadClass' in kwargs else ['residential'])
    ...
    **kwargs: optional
    ...
    if (shape == 'Poly'):
        ...
        for n in nodeIDs:
            ...
            poly = [[
                nodes[n][ptFieldName][0] + kwargs['radius'] * math.sin(2 * d * math.pi / lod),
                nodes[n][ptFieldName][1] + kwargs['radius'] * math.cos(2 * d * math.pi / lod),
        ...
        for n in nodeIDs:
            ...
            poly = [[
                nodes[n][ptFieldName][0] + radius * math.sin(2 * d * math.pi / lod),
                nodes[n][ptFieldName][1] + radius * math.cos(2 * d * math.pi / lod),
            ] for d in range(lod + 1)]
            nodes[n][neighborFieldName] = [poly[i] for i in range(len(poly)) if distEuclideanXY(poly[i], poly[i - 1]) > ERRTOL['distPt2Pt']]
        ...
        for n in nodeIDs:
            ...
            poly = [[
                nodes[n][ptFieldName][0] + (r[d] + kwargs['minRadius']) * math.sin(2 * d * math.pi / lod),
                nodes[n][ptFieldName][1] + (r[d] + kwargs['minRadius']) * math.cos(2 * d * math.pi / lod),
            ] for d in range(lod + 1)]
            nodes[n][neighborFieldName] = [poly[i] for i in range(len(poly)) if distEuclideanXY(poly[i], poly[i - 1]) > ERRTOL['distPt2Pt']]
